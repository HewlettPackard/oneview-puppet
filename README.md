...
**NOTE:** It should be noted that all information stored on the login file or json files should be in clear text, so to avoid security issues we strongly recommend verifying the access permissions for those files.
...
The majority of the ensurables on the resources require a `data` parameter to be informed, containing a hash with all the information required for the operation to be performed.

All `get_` ensurables output the information retrieved to the standard output.

As a way to provide easier deployment and management of infrastructure, all resources tags that either have or require a `Uri` can receive either the 'name' or a 'name, resource type' combination of parameters instead of the uri as the value to the field, unless otherwise specified.
...
**NOTE:**  This resource does NOT accept a `present`/`absent` state as it is created/removed through other HPE Oneview resources.
...
**NOTE:** This resource does not have an `absent` or `found` ensurables, it is only able to upload the SPPs/Hotfixes to the HPE Oneview appliance by using the `present` ensurable.
...
**NOTE:** This resource does not have an `absent` ensurable, and the `present` can only be used to perform patch operations. The creation/deletion of these resources is managed by other resource.
...
**NOTE:** This resource does not have a `present` and `absent` ensurables. The creation/deletion of these resources is managed by other resource.
...
* `present` - Updates the specified Logical Interconnect. **NOTE:** Creation is managed by other resources.
...
**NOTE:** This resource does not have an `absent` ensurable. The deletion of these resources is managed by other resource.
...
* `present` - Updates a Managed SAN. **Note:** Creation is managed by other resource.
...
**NOTE:** The creation of this resource is managed by the HPE Oneview itself when creating server hardwares. This provider can only update existing types and remove unused ones.
...
**NOTE:** The `present` ensurable is unavailable for this resource. Creation of this resource is managed by another resource on HPE Oneview.
...
**NOTE:** An unmanaged device is used to manually describe any device, such as a server, enclosure, storage array, networking switch, tape drive, or display that takes up space in a rack or consumes power but cannot otherwise be managed by the appliance.
...
**NOTE:** The `present` and `absent` ensurables are unavailable for this resource. Creation and deletion of this resource are managed by another resource on HPE Oneview.
