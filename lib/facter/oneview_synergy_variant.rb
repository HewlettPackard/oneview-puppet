...
Facter.add('oneview_synergy_variant') do
  setcode do
    ...
    def login
      begin
        # - Creating a JSON file with the proper fields and setting the environment variable ONEVIEW_SYNERGY_AUTH_FILE to its path
        credentials = if ENV['ONEVIEW_SYNERGY_AUTH_FILE']
                        JSON.parse(File.read(File.absolute_path(ENV['ONEVIEW_SYNERGY_AUTH_FILE'])))
                      # - Declaring each field as an environment variable
                      elsif ENV['ONEVIEW_SYNERGY_URL']
    ...
    def environment_credentials
      {
        'url' =>                     ENV['ONEVIEW_SYNERGY_URL'],
        'ssl_enabled' =>             ENV['ONEVIEW_SYNERGY_SSL_ENABLED'],
        'log_level' =>               ENV['ONEVIEW_SYNERGY_LOG_LEVEL'] || 'info',
        'api_version' =>             ENV['ONEVIEW_SYNERGY_API_VERSION'] || 200,
        'token' =>                   ENV['ONEVIEW_SYNERGY_TOKEN'] || nil,
        'user' =>                    ENV['ONEVIEW_SYNERGY_USER'] || nil,
        'password' =>                ENV['ONEVIEW_SYNERGY_PASSWORD'] || nil,
        'hardware_variant' =>        ENV['ONEVIEW_SYNERGY_HARDWARE_VARIANT'] || 'C7000'
      }
    end
