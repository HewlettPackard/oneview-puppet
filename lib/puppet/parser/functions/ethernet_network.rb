...
module Puppet::Parser::Functions
  newfunction(:ethernet_network) do |args|
    ...
    attributes_has_name = false
    attributes_has_name = true if attributes['name']
    new_network_exists = false
    new_matches = OneviewSDK::EthernetNetwork.find_by(@client, name: attributes['name'])
    new_network_exists = true if new_matches.first
    ...
    #new_matches = OneviewSDK::EthernetNetwork.find_by(@client, name: attributes['name']) if attributes['name']
    #puts new_matches if attributes['name']
    #new_network_exists = true if new_matches.first && action == "update" && attributes_exist && attributes['name']
    ...
    when "update"
      matches.first.update(attributes) if network_exists && attributes_exist && !new_network_exists
      puts "\n'attributes' not found. Please declare the attributes to be changed when using 'update'" if !attributes_exist
      puts "\nAn ethernet network named '#{options['name']}' does not exist.\n" if !network_exists
      puts "\nAn ethernet network named '#{attributes['name']}' already exists, cannot change existing name to one that is already created.\n" if new_network_exists
      puts "\nUpdated ethernet-network: '#{options["name"]}'.\n  uri = '#{matches.first[:uri]}'" if network_exists && attributes_exist && !new_network_exists
      puts "with attributes: #{attributes}\n" if network_exists && attributes_exist && !new_network_exists
